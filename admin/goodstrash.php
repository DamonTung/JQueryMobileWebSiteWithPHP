...
if( !isset($_SESSION['username'])||empty($_SESSION['username']) || !isset($_SESSION['user_id']) || empty($_SESSION['user_id']) ){
    ...
    } elseif (isset($_GET['act']) && $_GET['act'] == 'trash' && $goods_id) {
    ...
    } elseif (isset($_GET['act']) && $_GET['act'] == 'delete' && $goods_id) {
        if ($goods->delete($goods_id)) {
            header("refresh:5;url=goodslist.php");
            echo '成功彻底删除,5秒后自动返回商品列表。';
            echo '&nbsp;<span><a href="javascript:history.go(-1);">&#9666;返回上一步</a></span>';
        } else {
            echo '删除失败';
            echo '&nbsp;<span><a href="javascript:history.go(-1);">&#9666;返回上一步</a></span>';
        }
    } elseif (isset($_GET['act']) && $_GET['act'] == 'recovery' && $goods_id) {
        if ($goods->recovery($goods_id)) {
            header("refresh:5;url=goodslist.php");
            echo '恢复成功,5秒后自动返回商品列表。';
            echo '&nbsp;<span><a href="javascript:history.go(-1);">&#9666;返回上一步</a></span>';
        } else {
            echo '恢复失败';
            echo '&nbsp;<span><a href="javascript:history.go(-1);">&#9666;返回上一步</a></span>';
